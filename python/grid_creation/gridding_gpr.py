    ...
    while True:
        if pQueue.qsize() > 0:
            ...
            for beacon in beacons.keys():
                ...
                # arrange f(x1,x2)
                for rssi_index in range(len(bins)-1):
                    output_set = []
                    for p in dataHist.keys():
                        if not point == p:
                            output_set.append([dataHist[p][beacon][rssi_index]])

                    Y = np.array(output_set) # depends on the beacons and rssi values

                    xin = np.array([[point[0], point[1]]])
                    mean, sigma, K = gpr(X, Y, xin, K = K, sigma = sigma, kernel
                    = kernel_gauss_ebden, params = [sf, l, sn])
                    hist.append(max(0,mean[0,0]))

                hist = hist_normalize(hist)
